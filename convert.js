...
function createAdeptMap() {
  const dataPath = path.join(CWD, 'adept-maps');

  const adeptMap = {};

  var files = dir.files(dataPath, { sync: true });
  files.filter(fileName => fileName.endsWith('.jsonc')).forEach((fileName) => {
    const file = fs.readFileSync(fileName).toString();
    Object.assign(adeptMap, parse(file))
  })
...
function convertToAdept(roll, adeptMap) {
  const { hash } = roll;
...
function addAdeptRolls(rolls) {
  ...
  // just assigns name to the existing json data
  const adeptMap = createAdeptMap();
  const adepts = [];


  rolls.forEach(roll => {
    const adeptRoll = convertToAdept(roll, adeptMap);
